...
function TestimonialCard({ t }: { t: (typeof TESTIMONIALS)[number] }) {
  return (
    <div
      className="rounded-2xl p-5 sm:p-6 flex flex-col h-full transition-all duration-300 hover:shadow-xl hover:-translate-y-1 hover:border-lime/20"
      style={GLASS_CARD}
    >
      {t.featured && (
...
export default function PilotTestimonials() {
  ...
  useSectionVisibility("pilot_testimonials", containerRef);
  const testimonialNames = TESTIMONIALS.map((t) => t.name);
  const { trackTransition } = useCarouselTracking("pilot_testimonials", activeDot, testimonialNames);
  ...
  useEffect(() => {
    ...
    const onScroll = () => {
      ...
      if (newIndex !== lastIndex) {
        trackTransition(newIndex, "swipe");
        lastIndex = newIndex;
      }
  ...
  return (
    <section ref={containerRef} data-section-name="pilot_testimonials" className="relative py-12 sm:py-16 lg:py-20 overflow-hidden" style={{ backgroundColor: "#FEFCFA" }}>
      {/* Amplified background glow */}
      <div
        className="absolute inset-0 pointer-events-none"
        style={{
          background:
            "radial-gradient(ellipse 80% 60% at 50% 35%, rgba(255,138,76,0.09), transparent 65%)",
        }}
      />
