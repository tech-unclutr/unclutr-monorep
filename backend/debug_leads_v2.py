...
async def debug_leads():
    async for session in get_session():
        ...
        if not campaign:
            ...
            # Let's try to get the most recent one
            stmt = select(Campaign).order_by(Campaign.updated_at.desc()).limit(1)
            result = await session.execute(stmt)
            campaign = result.scalars().first()
            if not campaign:
                return
            print(f"Using most recent campaign: {campaign.name} (ID: {campaign.id}, Status: {campaign.status})")
        else:
            print(f"Active Campaign: {campaign.name} (ID: {campaign.id})")

        # 2. Check for "Param" and "Kunj" in leads
        print("\n--- Lead Status ---")
