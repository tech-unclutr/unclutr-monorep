...
async def show_lead_payload(campaign_id_str, customer_name_search):
    async with AsyncSession(engine) as session:
        ...
        lead_stmt = select(CampaignLead).where(
            ...
            CampaignLead.customer_name.ilike(f"%{customer_name_search}%")
        )
        result = await session.execute(lead_stmt)
        lead = result.scalars().first()
        
        if not lead:
            print(f"Lead '{customer_name_search}' not found in campaign {campaign_id_str}")
            return
        ...
        print(f"\nTarget Lead: {lead.customer_name}")
        print(f"Cohort: {lead.cohort}")
        print(f"\nFinal Payload JSON for Bolna:")
        print(json.dumps(variables, indent=2))

if __name__ == "__main__":
    asyncio.run(show_lead_payload("7b277bac-9157-4c01-9b6e-3b28b088e0b4", "Kunj"))
