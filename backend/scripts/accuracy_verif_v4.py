...
async def test_accuracy_verif_v4():
    # USE EXISTING COMPANY TO AVOID FK HELL
    COMPANY_ID = uuid.UUID("017ac5e1-78fc-438f-813f-ffc9acc18c14")
    
    async with async_session_factory() as session:
        uhex = uuid.uuid4().hex
        print(f"Starting Verif V4 (Company: {COMPANY_ID})")
        
        # 1. SETUP
        # Create Brand
        brand_id = uuid.uuid4()
        brand = Brand(id=brand_id, name=f"V4 Brand {uhex[:8]}", company_id=COMPANY_ID)
        session.add(brand)
        
        # Create DataSource
        ds_id = uuid.uuid4()
        ds = DataSource(id=ds_id, name=f"V4 DS {uhex[:8]}", slug=f"v4-audit-{uhex}", category="Storefront")
        session.add(ds)
        
        # Create Workspace
        ws_id = uuid.uuid4()
        workspace = Workspace(id=ws_id, name=f"V4 WS {uhex[:8]}", brand_id=brand_id, company_id=COMPANY_ID)
        ...
        order1 = ShopifyOrder(
            ...
            company_id=COMPANY_ID,
            shopify_order_id=uuid.uuid4().int & (1<<61)-1, # Small random
            shopify_order_number=4001,
            shopify_name="#4001",
        ...
        order2 = ShopifyOrder(
            ...
            integration_id=int_id,
            company_id=COMPANY_ID,
            shopify_order_id=uuid.uuid4().int & (1<<61)-1,
            shopify_order_number=4002,
            shopify_name="#4002",
            financial_status="paid",
        ...
        refund = ShopifyTransaction(
            ...
            integration_id=int_id,
            company_id=COMPANY_ID,
            shopify_transaction_id=uuid.uuid4().int & (1<<61)-1,
            order_id=order1.id,
            kind="refund",
            status="success",
...
if __name__ == "__main__":
    asyncio.run(test_accuracy_verif_v4())
