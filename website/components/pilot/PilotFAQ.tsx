...
export default function PilotFAQ() {
  ...
  useSectionVisibility("pilot_faq", containerRef);

  const toggle = (index: number) => {
    const isOpening = openIndex !== index;
    setOpenIndex((prev) => (prev === index ? null : index));
    trackEvent(isOpening ? EventName.CAROUSEL_INTERACT : EventName.CAROUSEL_INTERACT, {
      carousel_id: "pilot_faq",
      action: isOpening ? "open" : "close",
      faq_index: index,
      faq_question: FAQS[index].q.slice(0, 80),
    } as Record<string, string | number>);
  };

  return (
    <section ref={containerRef} data-section-name="pilot_faq" id="faq" className="py-12 sm:py-16 lg:py-20" style={{ backgroundColor: "#f5f2ed" }}>
        ...
        <div className="space-y-3">
          {FAQS.map((faq, i) => (
            <Reveal key={i} width="100%" delay={0.1 + i * 0.05}>
              <div className={`rounded-2xl bg-white border overflow-hidden transition-all duration-200 ${openIndex === i ? "border-lime/30 shadow-md" : "border-neutral-200 hover:border-lime/20 hover:shadow-sm"}`}>
                <button
                  onClick={() => toggle(i)}
